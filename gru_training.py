...
def train_gru(model, iterator, optimizer, criterion, device):
    model.train()
    epoch_loss = 0
    epoch_acc = 0
...
def evaluate_gru(model, iterator, criterion, device):
    model.eval()
    epoch_loss = 0
    epoch_acc = 0
...
def main():
    ...
    joblib.dump(model_config, os.path.join(model_dir, 'gru_config.pkl'))
    ...
    for epoch in range(N_EPOCHS):
        train_loss, train_acc = train_gru(model, train_loader, optimizer, criterion, device)
        val_loss, val_acc, accuracy, precision, recall, f1 = evaluate_gru(model, test_loader, criterion, device)
        
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save(model.state_dict(), os.path.join(model_dir, 'best_gru_model.pt'))
    ...
    torch.save(model.state_dict(), os.path.join(model_dir, 'final_gru_model.pt'))
    
    torch.save(model, os.path.join(model_dir, 'complete_gru_model.pt'))
    
    model.load_state_dict(torch.load(os.path.join(model_dir, 'best_gru_model.pt')))
    
    val_loss, val_acc, accuracy, precision, recall, f1 = evaluate_gru(model, test_loader, criterion, device)
    
    gru_time = time.time() - start_time
    
    print(f"\nGRU Training Time: {gru_time:.2f} seconds")
    print(f"Final Accuracy: {accuracy:.4f}")
    print(f"Final Precision: {precision:.4f}")
    print(f"Final Recall: {recall:.4f}")
    print(f"Final F1 Score: {f1:.4f}")

    results_file = os.path.join(model_dir, 'gru_evaluation_results.txt')
    with open(results_file, 'w') as f:
        f.write(f"SVM Training Time: {gru_time:.2f} seconds\n")
